...
const Panjabi = () => {
  ...
  const filterProducts = (products) => {
    return products.filter((product) => product.category === "Panjabi");
  };
  ...
  const handleRequestSort = async (data) => {
    ...
    try {
      ...
      setProductToShow(data?.filter((curElem) => curElem.category === "Ss23"));
    } catch (error) {
      console.error("Error fetching filtered products:", error);
    } finally {
      // setIsFetching(false);
  ...
  useEffect(() => {
    if (products && !isLoading) {
      setProductToShow(
        products.filter((curElem) => curElem.category === "Panjabi")
      );
    }
  }, [products]);
  ...
  return (
    <div className=" px-6 lg:px-[4rem] xl:px-[4rem] pt-[2.13rem] pb-[3rem] lg:pt-[5.19rem] lg:pb-[4.35rem]">
      <MobileFilter
      ...
      <div className="mb-[1.3rem] md:mb-[1.5rem] flex items-center justify-between">
        ...
        <Sort handleRequestSort={handleRequestSort} Page={"panjabi"} />
      </div>
